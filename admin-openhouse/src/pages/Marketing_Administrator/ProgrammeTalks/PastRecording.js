  ...
  constructor() {
    ...
    display= () => {
        ...
        .then((snapshot) => {
            ...
            db.collection("ProgrammeTalks").where("date", "==", dates[0].date).where("hasRecording", "==", true).get()
            .then((snapshot) => {
                const pastRecording = [];
                snapshot.forEach((doc) => {
                    const data = {
            ...
            db.collection("ProgrammeTalks").where("date", "==", dates[1].date).where("hasRecording", "==", true).get()
            .then((snapshot) => {
                const pastRecording = [];
                snapshot.forEach((doc) => {
                    const data = {
    ...
    DeletePastRecording(e, pastrecordingid) {
        db.collection("ProgrammeTalks").doc(pastrecordingid).delete()
        .then(() => {
            this.display();
