  ...
  state = initialStates;
    ...
    display= () => {
        ...
        .then((snapshot) => {
            ...
            db.collection("ProgrammeTalks").where("date", "==", dates[0].date).where("isLive", "==", true).get()
            .then((snapshot) => {
                const livetalk = [];
                snapshot.forEach((doc) => {
                    const data = {
            ...
            db.collection("ProgrammeTalks").where("date", "==", dates[1].date).where("isLive", "==", true).get()
            .then((snapshot) => {
                const livetalk = [];
                snapshot.forEach((doc) => {
                    const data = {
    ...
    addLiveTalks = (e) => {
        ...
        if (isValid) {
            ...
            db.collection("ProgrammeTalks").orderBy('id','desc').limit(1).get()
            .then((snapshot) =>  {
                snapshot.forEach((doc) => {
                    var docid= "";
                    ...
                    db.collection("ProgrammeTalks").doc(docid)
                    .set({
    ...
    DeleteLiveTalk(e, livetalkid) {
        db.collection("ProgrammeTalks").doc(livetalkid).delete()
        .then(() => {
            this.display();
    ...
    editLiveTalk(e, livetalkid) {
        ...
        if (isValid) {
            ...
            db.collection("ProgrammeTalks").doc(this.state.id)
            .update({
                awardingUni: this.state.awardingUni,
                endTime: this.state.endTime,
                startTime: this.state.startTime,
